...
║  All polling bugs fixed, optimized & stable               ║
╚═══════════════════════════════════════════════════════════╝
"""
...
# ═══════════════════════════════════════════════════
#  CONFIGURATION (FIX #1: Token Verification)
# ═══════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════
#  FIX #2: TOKEN VALIDATION BEFORE BOT CREATION
# ═══════════════════════════════════════════════════
def validate_token(token):
    """Validate bot token before creating bot instance"""
    if not token:
        logger.error("❌ BOT_TOKEN is empty or not set!")
        logger.error("Set it: export BOT_TOKEN='your_token_here'")
        return False
    if ':' not in token:
        logger.error("❌ BOT_TOKEN format invalid! Expected: 123456:ABCdef...")
        return False
    try:
        resp = requests.get(f"https://api.telegram.org/bot{token}/getMe", timeout=10)
        if resp.status_code == 200 and resp.json().get('ok'):
            bot_info = resp.json()['result']
            logger.info(f"✅ Token valid! Bot: @{bot_info.get('username', '?')}")
            return True
        else:
            logger.error(f"❌ Token invalid! API response: {resp.text[:200]}")
            return False
    except requests.exceptions.ConnectionError:
        logger.warning("⚠️ Can't verify token (no internet), proceeding anyway...")
        return True
    except Exception as e:
        logger.warning(f"⚠️ Token check error: {e}, proceeding...")
        return True

if not validate_token(TOKEN):
    logger.critical("❌ FATAL: Invalid BOT_TOKEN! Exiting...")
    sys.exit(1)
...
# ═══════════════════════════════════════════════════
#  FIX #3: INSTANCE LOCK (Prevent Multiple Instances)
# ═══════════════════════════════════════════════════
            ...
            if psutil.pid_exists(old_pid):
                try:
                    ...
                    if 'python' in p.name().lower():
                        logger.warning(f"⚠️ Another instance running (PID: {old_pid}), killing it...")
                        p.terminate()
                        p.wait(timeout=5)
...
# ═══════════════════════════════════════════════════
#  FIX #4: WEBHOOK CLEANER THREAD
# ═══════════════════════════════════════════════════
def thread_webhook_cleaner():
    """Periodically ensure no webhook is set (prevents conflict)"""
    while True:
        try:
            ...
            try:
                ...
                if webhook_info.url:
                    ...
                    bot.remove_webhook()
                    logger.info("✅ Webhook removed by cleaner")
            except:
                pass
        except:
            pass
...
# ═══════════════════════════════════════════════════
#  FIX #5: SAFE POLLING WITH ALL FIXES
# ═══════════════════════════════════════════════════
def remove_webhook_safe():
    """Remove webhook with multiple retry attempts"""
    for attempt in range(3):
        try:
            ...
            time.sleep(2)
    logger.warning("⚠️ Could not remove webhook, proceeding anyway...")
    return False


def delete_pending_updates():
    """Delete all pending updates to avoid processing old messages"""
    try:
        url = f"https://api.telegram.org/bot{TOKEN}/getUpdates"
        resp = requests.get(url, params={'offset': -1, 'timeout': 1}, timeout=10)
        if resp.status_code == 200:
            data = resp.json()
            if data.get('ok') and data.get('result'):
                last_update_id = data['result'][-1]['update_id']
                requests.get(url, params={'offset': last_update_id + 1, 'timeout': 1}, timeout=10)
                logger.info(f"✅ Cleared pending updates (last: {last_update_id})")
            else:
...
def safe_polling():
    """Start polling with all fixes applied"""
    
    # Step 1: Remove webhook
    logger.info("🔧 Step 1: Removing webhook...")
    remove_webhook_safe()
    
    # Step 2: Clear pending updates
    logger.info("🔧 Step 2: Clearing pending updates...")
    delete_pending_updates()
    
    # Step 3: Small delay
    time.sleep(2)
    
    # Step 4: Start polling with auto-restart
    logger.info("🚀 Step 3: Starting safe polling...")
    
    retry_count = 0
    max_retries = 50  # Max retries before longer wait
    ...
    while True:
        try:
            ...
            logger.error(f"🔴 Connection error (retry {retry_count}): {e}")
            logger.info(f"🔄 Reconnecting in {wait}s...")
            time.sleep(wait)
        except requests.exceptions.ReadTimeout as e:
            retry_count += 1
            logger.error(f"🔴 Read timeout (retry {retry_count}): {e}")
            time.sleep(3)
        except telebot.apihelper.ApiTelegramException as e:
            err_str = str(e).lower()
            retry_count += 1
            ...
            if 'conflict' in err_str:
                logger.error("🔴 CONFLICT: Another instance is running!")
                logger.info("🔧 Removing webhook and retrying in 10s...")
                remove_webhook_safe()
                delete_pending_updates()
                time.sleep(10)
            elif 'unauthorized' in err_str or '401' in err_str:
                logger.critical("❌ FATAL: Bot token is invalid/revoked!")
                break
                ...
                wait = min(30, 5 * retry_count)
                logger.error(f"🔴 API error (retry {retry_count}): {e}")
                time.sleep(wait)
        except Exception as e:
            retry_count += 1
            wait = min(60, 5 * retry_count)
            logger.error(f"🔴 Unexpected error (retry {retry_count}): {e}", exc_info=True)
            
            if retry_count >= max_retries:
                logger.warning(f"⚠️ {max_retries} retries reached, waiting 120s...")
                time.sleep(120)
                retry_count = 0
                # Re-remove webhook after long wait
                remove_webhook_safe()
                delete_pending_updates()
            else:
                time.sleep(wait)
...
# ═══════════════════════════════════════════════════
#  MAIN (ALL FIXES APPLIED)
# ═══════════════════════════════════════════════════
def main():
    logger.info("=" * 50)
    logger.info(f"  {BRAND} {BRAND_VER}")
    logger.info(f"  All Polling Fixes Applied ✅")
    logger.info("=" * 50)

    # FIX: Check single instance
    ...
    threading.Thread(target=thread_webhook_cleaner, daemon=True, name="WebhookCleaner").start()
    ...
    logger.info("🟢 Bot READY! Starting safe polling...")

    # FIX #5: Use safe_polling instead of direct infinity_polling
    safe_polling()
